...
func (pe *PenaltyController) Create(c *gin.Context) { //Hay que ingresar todos los datos necesarios para crear
	...
	err = pe.PenaltyRepository.Create(c, Penalty)
	if err != nil {
...
func (pe *PenaltyController) Fetch(c *gin.Context) {
	Penaltys, err := pe.PenaltyRepository.Fetch(c)
	if err != nil {
		c.JSON(http.StatusInternalServerError, domain.ErrorResponse{Message: err.Error()})
		return
	}
...
func (pe *PenaltyController) FetchById(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		c.JSON(http.StatusInternalServerError, domain.ErrorResponse{Message: err.Error()})
		return
	}
	Penaltys, err := pe.PenaltyRepository.FetchById(c, id)
	if err != nil {
		c.JSON(http.StatusInternalServerError, domain.ErrorResponse{Message: err.Error()})
		return
	}
...
func (pe *PenaltyController) Update(c *gin.Context) {
	...
	err = pe.PenaltyRepository.Update(c, *updatedPenalty)
	if err != nil {
		c.JSON(http.StatusInternalServerError, domain.ErrorResponse{Message: err.Error()})
	}
...
func (pe *PenaltyController) Delete(c *gin.Context) {
	PenaltyID, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		c.JSON(http.StatusInternalServerError, domain.ErrorResponse{Message: err.Error()})
	}
	err = pe.PenaltyRepository.Delete(c, PenaltyID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, domain.ErrorResponse{Message: err.Error()})
	}
